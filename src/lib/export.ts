...
export function exportLoanCSV(data: LoanExportData) {
  ...
  if (data.schedule.length > 0) {
    ...
    lines.push(
      "Year,Payment,Principal,Interest,Balance,Total Principal,Total Interest"
    );
...
export async function exportLoanExcel(data: LoanExportData) {
  ...
  titleCell.font = {
    size: 24,
    bold: true,
    color: { argb: COLORS.charcoal.slice(2) }
  };
  ...
  summary.getCell("B3").font = {
    size: 10,
    italic: true,
    color: { argb: COLORS.slate.slice(2) }
  };
  ...
  highlightCell.value = {
    richText: [
      ...
      {
        text: formatCurrency(data.monthlyPayment),
        font: { size: 28, bold: true, color: { argb: "FFFFFF" } }
      }
    ]
  };
  highlightCell.fill = {
    type: "pattern",
    pattern: "solid",
    fgColor: { argb: COLORS.terracotta.slice(2) }
  };
  highlightCell.alignment = {
    horizontal: "center",
    vertical: "middle",
    wrapText: true
  };
  ...
  detailsHeader.fill = {
    type: "pattern",
    pattern: "solid",
    fgColor: { argb: COLORS.cream.slice(2) }
  };
  detailsHeader.border = {
  ...
  for (const [label, value] of details) {
    ...
    summary.getCell(`B${row}`).font = {
      size: 10,
      color: { argb: COLORS.slate.slice(2) }
    };
    summary.getCell(`C${row}`).value = value;
    summary.getCell(`C${row}`).font = {
      size: 10,
      bold: true,
      color: { argb: COLORS.charcoal.slice(2) }
    };
  ...
  const principalPct =
    data.totalPayment > 0
      ? ((data.principal / data.totalPayment) * 100).toFixed(1)
      : "0.0";
  const interestPct =
    data.totalPayment > 0
      ? ((data.totalInterest / data.totalPayment) * 100).toFixed(1)
      : "0.0";
  ...
  summary.getCell("E9").font = {
    size: 11,
    bold: true,
    color: { argb: COLORS.charcoal.slice(2) }
  };
  summary.getCell("E9").fill = {
    type: "pattern",
    pattern: "solid",
    fgColor: { argb: COLORS.cream.slice(2) }
  };
  summary.getCell("E9").border = {
    bottom: { style: "thin", color: { argb: COLORS.sand.slice(2) } }
  };
  summary.getCell("E9").alignment = { horizontal: "center" };

  summary.getCell("E11").value = "Principal";
  summary.getCell("E11").font = {
    size: 9,
    color: { argb: COLORS.slate.slice(2) }
  };
  ...
  summary.getCell("E12").fill = {
    type: "pattern",
    pattern: "solid",
    fgColor: { argb: COLORS.charcoal.slice(2) }
  };
  summary.getRow(12).height = 8;

  summary.getCell("E14").value = "Interest";
  summary.getCell("E14").font = {
    size: 9,
    color: { argb: COLORS.slate.slice(2) }
  };
  ...
  if (data.schedule.length > 0) {
    ...
    headerRow.font = {
      size: 10,
      bold: true,
      color: { argb: COLORS.white.slice(2) }
    };
    headerRow.fill = {
      type: "pattern",
      pattern: "solid",
    ...
    data.schedule.forEach((item, index) => {
      ...
      if (index % 2 === 0) {
        dataRow.fill = {
          type: "pattern",
          pattern: "solid",
          fgColor: { argb: COLORS.ivory.slice(2) }
        };
      }
    });
...
export function exportMortgageCSV(data: MortgageExportData) {
  ...
  for (const cost of data.customCosts) {
    if (cost.monthlyAmount > 0) {
      lines.push(
        `${cost.name || "Other"},${formatCurrency(cost.monthlyAmount)}`
      );
    }
  }
...
export async function exportMortgageExcel(data: MortgageExportData) {
  ...
  if (data.monthlyHOA > 0)
    monthlyCosts.push(["HOA", data.monthlyHOA, "FFd97b5d"]);
  data.customCosts.forEach((cost, idx) => {
    if (cost.monthlyAmount > 0) {
      monthlyCosts.push([
        cost.name || "Other",
        cost.monthlyAmount,
        customCostColors[idx % customCostColors.length]
      ]);
    }
  });
  ...
  for (const [label, value, color] of monthlyCosts) {
    const pct =
      data.totalMonthly > 0
        ? ((value / data.totalMonthly) * 100).toFixed(1)
        : "0.0";
    summary.getCell(`B${row}`).value = label;
    summary.getCell(`B${row}`).font = {
      size: 10,
  ...
  summary.getCell("E5").font = {
    size: 11,
    bold: true,
    color: { argb: COLORS.charcoal.slice(2) }
  };
  summary.getCell("E5").fill = {
    type: "pattern",
    pattern: "solid",
    fgColor: { argb: COLORS.cream.slice(2) }
  };
  summary.getCell("E5").border = {
    bottom: { style: "thin", color: { argb: COLORS.sand.slice(2) } }
  };
  ...
  for (const [label, value, color] of monthlyCosts) {
    const pct =
      data.totalMonthly > 0
        ? ((value / data.totalMonthly) * 100).toFixed(0)
        : "0";

    // Color indicator
    summary.getCell(`E${chartRow}`).fill = {
      type: "pattern",
      pattern: "solid",
      fgColor: { argb: color.slice(2) }
    };
    ...
    summary.getCell(`F${chartRow}`).font = {
      size: 9,
      color: { argb: COLORS.charcoal.slice(2) }
    };
    chartRow++;
  ...
  summary.getCell(`E${chartRow + 1}`).font = {
    size: 11,
    bold: true,
    color: { argb: COLORS.charcoal.slice(2) }
  };
  summary.getCell(`E${chartRow + 1}`).fill = {
    type: "pattern",
    pattern: "solid",
    fgColor: { argb: COLORS.cream.slice(2) }
  };
  summary.getCell(`E${chartRow + 1}`).border = {
    bottom: { style: "thin", color: { argb: COLORS.sand.slice(2) } }
  };
  chartRow += 2;

  summary.getCell(`E${chartRow}`).value = "Total Interest";
  summary.getCell(`E${chartRow}`).font = {
    size: 10,
    color: { argb: COLORS.slate.slice(2) }
  };
  ...
  summary.getCell(`F${chartRow}`).font = {
    size: 12,
    bold: true,
    color: { argb: COLORS.terracotta.slice(2) }
  };
  summary.getCell(`F${chartRow}`).alignment = { horizontal: "right" };
...
export async function exportInvestmentExcel(data: InvestmentExportData) {
  ...
  highlightCell.value = {
    richText: [
      { text: "FUTURE VALUE\n", font: { size: 10, color: { argb: "FFFFFF" } } },
      {
        text: formatCurrency(data.futureValue),
        font: { size: 28, bold: true, color: { argb: "FFFFFF" } }
      }
    ]
  };
  ...
  const contribPct =
    data.futureValue > 0
      ? ((data.totalContributions / data.futureValue) * 100).toFixed(1)
      : "0.0";
  const interestPct =
    data.futureValue > 0
      ? ((data.totalInterest / data.futureValue) * 100).toFixed(1)
      : "0.0";
  ...
  summary.getCell(`C${row}`).value =
    `${formatCurrency(data.totalContributions)} (${contribPct}%)`;
  summary.getCell(`C${row}`).font = {
    size: 10,
    bold: true,
    color: { argb: COLORS.charcoal.slice(2) }
  ...
  summary.getCell(`C${row}`).value =
    `${formatCurrency(data.totalInterest)} (${interestPct}%)`;
  summary.getCell(`C${row}`).font = {
    size: 10,
    bold: true,
    color: { argb: COLORS.charcoal.slice(2) }
  ...
  await downloadWorkbook(
    workbook,
    `investment-projection-${formatDate()}.xlsx`
  );
}
